...
This vignette demonstrates the main function of the `furniture` package--`table1()`. This vignette is current as of `furniture` `v 1.6.1`.

The main parts of the `table1()` are below:

```{r structure, eval=FALSE}
table1(.data, ..., splitby, row_wise, test, type, output, format_number, NAkeep)
```
...
table1(df,
       "New A var" = factor(ifelse(a > 1, 1, 0)), b, c,
       splitby = ~d,
       test = TRUE)
```
